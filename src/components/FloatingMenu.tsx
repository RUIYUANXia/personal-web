...
const FloatingMenuWrapper = styled.div<{ isOpen: boolean; isLightTheme: boolean }>`
    ...
    .menu-item {
        ...
        background-color: ${({ isLightTheme }) =>
            isLightTheme ? colors.textTitleLight : colors.textTitleDark};
...
const Tooltip = styled.div<{ isLightTheme: boolean }>`
    ...
    background-color: ${({ isLightTheme }) =>
        isLightTheme ? colors.textTitleLight : colors.textTitleDark};
    color: ${({ isLightTheme }) =>
        isLightTheme ? colors.textBodyDark : colors.textBodyLight};
    padding: 3px 6px;
    border-radius: 3px;
    font-size: 12px;
    opacity: 0;
...
const MainButton = styled.div<{ isLightTheme: boolean; isOpen: boolean }>`
    ...
    &:hover {
        background-color: ${({ isLightTheme }) =>
            isLightTheme ? colors.textTitleDark : colors.textTitleLight};
    ...
    &::before {
        ...
        transform: translate(-50%, -50%)
            rotate(${({ isOpen }) => (isOpen ? '-90deg' : '0deg')});
        transition: transform 0.3s ease;
        font-size: 24px;
        color: ${({ isLightTheme }) =>
            isLightTheme ? colors.textTitleDark : colors.textTitleLight};
